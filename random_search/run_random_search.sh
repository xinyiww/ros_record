...
ROSBAG=/home/xliu/Documents/ros_record/random_search/t_jun_pred_combo_2_filtered.bag
# working space
...
# Skip the header row
tail -n +1 "$CSV_FILE" | while IFS=, read -r id max_acc min_d_gap min_t_gap; do
  ...
  rosbag record -e "/region/all_cars_predictions_idm" -O params_random_search_pred_${id}.bag __name:=my_bag & </dev/null
